              ...
              <ul className="space-y-1">
                {Object.entries(filteredAttrs).map(([key, val]) => (
                  <li key={key} className="flex justify-between border-b border-gray-600/30 pb-1">
                    <span className={`flex items-center gap-2 px-3 py-2 rounded-xl ${darkMode ? "bg-custom-blue-600 hover:bg-blue-700" : "bg-custom-blue-500 hover:bg-blue-600"} text-white`}>{key}</span>
                    <span className="flex items-center px-2 py-1">{String(val?.value ?? "")}</span>
                    {val?.type && <span className={`flex items-center gap-2 px-2 py-2 rounded-xl text-gray-400 ml-2 ${darkMode ? "bg-zinc-600" : "bg-zinc-300"}`}>({val.type})</span>}
                  </li>
                ))}
              </ul>
            </div>
